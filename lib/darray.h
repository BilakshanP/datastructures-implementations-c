...
/// @return Pointer to the newly constructed `DArray_t`, or `NULL` on allocation failure.
...
/// @return Pointer to the newly constructed `DArray_t`, or `NULL` on failure. The resulting array's capacity will be equal to `length`.
...
/// @return Pointer to the newly constructed copy of `DArray_t`, or `NULL` on failure.
...
/// @details This includes deallocating the internal elements via the `deallocator`, freeing the underlying array storage, and finally freeing the `DArray_t` structure itself.
...
size_t da_length(const DArray_t* da);
...
void* da_index(DArray_t* da, size_t idx);
...
/// @details This pointer is still managed by the `DArray_t` structure. Do not manually free this pointer.
/// @param da Pointer to the dynamic array.
/// @return Pointer to the inner array (`da->arr`).
void* da_raw(DArray_t* da);

/// @brief Unwraps and returns the raw underlying array pointer, and frees the `DArray_t` structure itself.
...
/// @return Pointer to the newly created subarray `DArray_t`, or `NULL` on error (e.g., invalid indices or allocation failure).
